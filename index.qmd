...
``` {style="color:green"}
 withr::with_libpaths("..lib.path...",
                         devtools::install_local(file_zip))
```
...
Récupération sql depuis **dbplyr**

``` {style="color:green"}
tbl(xx,yy) |>   left_join(...)  |>  remote_query 
```
...
suppression doublon version dplyr/data.table

``` {style="color:green"}
# dplyr -----------------------------------------------------

cars |>  distinct(speed, .keep_all = TRUE)->cars

# data.table ....
cars[!duplicated(speed),]
```
...
``` {style="color:green"}
grep --color='auto' -P -n "[\x80-\xFF]" ~/chemin/*
...
SQL TOP 1 par groupe

``` {style="color:green"}

"SELECT *
  FROM (SELECT cnaem, num_pens,mois_update,mois_insert,
        dense_rank() over (partition by concat(cnaem,num_pens)
                           order by mois_update desc,
                           mois_insert desc) rnk
        FROM presta.presta_id) as foo
WHERE rnk = 1"
```
...
DataViz : <https://www.data-to-viz.com/>

Color palette : <https://r-graph-gallery.com/color-palette-finder>
